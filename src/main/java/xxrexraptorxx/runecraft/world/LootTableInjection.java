...
import xxrexraptorxx.runecraft.utils.mixins.LootTableAccessor;
...
public class LootTableInjection {
        ...
        if (name.startsWith(prefix)) {
            ...
            if (INJECTION_TABLES.containsKey(file)) {
                try {
                    ...
                    RuneCraft.LOGGER.error("Loottable {} is broken by some other mod. Cannot add " + References.NAME + " loot to it. " + name);
                }
            }
        }
    }
    ...
    @SubscribeEvent
    public static void onEntityLootLoad(@NotNull LootTableLoadEvent event) {
